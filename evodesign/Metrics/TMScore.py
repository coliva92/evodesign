...
class TMScore(StructuralMetric):
    ...
    def do(
        ...
        fragment_lengths = sorted(list(set([l for l in raw_lengths if l >= 3])), reverse=True)
        ...
        for L_int in fragment_lengths:
            ...
            for start in start_positions:
                ...
                while True:
                    ...
                    distances = np.array([np.linalg.norm(a.coord - b.coord) for a, b in zip(model_ca_atoms, ref_ca_atoms)])
                ...
                current_tm_score = self._calculate_total_tm_score(model_ca_atoms, ref_ca_atoms, d_0, L_T)
