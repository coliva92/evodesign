...
class ESM2(ESM2Interface):
    ...
    def query_model(
        ...
        aa = torch.from_numpy(
            np.array([OPENFOLD_MAP_AMINO_ACID_TO_INT[i] for i in sequence])
        )
        ...
        with torch.no_grad():
            ...
            s_z_0 = s_s_0.new_zeros(
                1, L, L, self._model.esmfold_model.cfg.trunk.pairwise_state_dim
            )
            recycle_z = torch.zeros_like(s_z_0)
            recycle_s = torch.zeros_like(s_s_0)
            recycle_bins = torch.zeros(
                *s_z_0.shape[:-1], device=self.gpu_device, dtype=torch.int64
            )
            recycle_s = self._model.esmfold_model.trunk.recycle_s_norm(
                recycle_s.detach()
            )
            recycle_z = self._model.esmfold_model.trunk.recycle_z_norm(
                recycle_z.detach()
            )
            recycle_z += self._model.esmfold_model.trunk.recycle_disto(
                recycle_bins.detach()
            )
            s_s_0 += recycle_s
            z = s_z_0 + recycle_z
            s_z_0 = z + self._model.esmfold_model.trunk.pairwise_positional_embedding(
                residx, mask=None
            )
